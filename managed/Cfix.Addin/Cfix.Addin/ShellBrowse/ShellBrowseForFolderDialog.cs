...
namespace Cfix.Addin.ShellBrowse
{
	public class ShellBrowseForFolderDialog
	{
		...
		private IFolderFilter filter;

		public IFolderFilter Filter
		{
			get { return filter; }
			set { filter = value; }
		}
		...
		private void ApplyFolderFilter( IntPtr hwnd, IntPtr iunknown )
		{
			...
			if ( this.filter != null )
			{
				System.Runtime.InteropServices.Marshal.QueryInterface( iunknown, ref ShellGUIDs.IID_IFolderFilterSite, out iFolderFilterSite );

				Object obj = System.Runtime.InteropServices.Marshal.GetTypedObjectForIUnknown( iFolderFilterSite, typeof(IFolderFilterSite) );
				IFolderFilterSite folderFilterSite = ( IFolderFilterSite ) obj;

				folderFilterSite.SetFilter( this.filter );
			}
		}
		...
		private Int32 BrowseCallbackProc(IntPtr hwnd, UInt32 uMsg, Int32 lParam, Int32 lpData)
		{
			switch ((BrowseForFolderMessages)uMsg)
			{
				...
				case BrowseForFolderMessages.BFFM_IUNKNOWN:
					...
					ApplyFolderFilter(hwnd,(IntPtr)lParam);
		...
		public void ShowDialog()
		{
			...
			bi.lParam = 0;
			bi.lpfn = new ShellApi.BrowseCallbackProc(this.BrowseCallbackProc);
			
			// Show dialog
			IntPtr pidlSelected;
			pidlSelected = Cfix.Addin.ShellBrowse.ShellApi.SHBrowseForFolder(ref bi);
			...
			String sDisplay;
			Cfix.Addin.ShellBrowse.ShellApi.StrRetToBSTR(ref ptrDisplayName,pidlRoot,out sDisplay);
			m_FullName = sDisplay;
